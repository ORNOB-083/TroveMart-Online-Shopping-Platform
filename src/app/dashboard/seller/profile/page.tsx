...
export default function SellerProfilePage() {
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const uploadToast = 'seller-upload';
    ...
    const modalPreviewSrc = pendingImage || savedImage || `https://ui-avatars.com/api/?name=${encodeURIComponent(name || 'U')}&background=0F766E&color=fff&size=96`;
    const profileImageSrc = savedImage || `https://ui-avatars.com/api/?name=${encodeURIComponent(name || 'U')}&background=0F766E&color=fff&size=96`;
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6">
            <motion.div
                ...
                className="space-y-1"
            >
                <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Vendor Profile</h1>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                    Manage your seller account details and storefront photo.
                </p>
            </motion.div>
                ...
                <div className="px-6 pb-6">
                    <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 -mt-12 mb-5">
                        <div className="relative inline-block">
                            <img
                                src={profileImageSrc}
                                alt={name || user?.name || 'Vendor Profile'}
                        ...
                        <div className="flex items-center gap-2 pb-1">
                            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800">
                                ...
                                <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400">Verified Vendor</span>
                            </div>
                            <span className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800">
                                Seller
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="space-y-1">
                            ...
                            {isEditingName ? (
                                <div className="flex items-center gap-2 flex-1">
                                    <input
                                        ...
                                        className="flex-1 px-3 py-1.5 text-lg font-bold bg-gray-50 dark:bg-gray-800 border border-emerald-300 rounded-xl text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                                        autoFocus
                                    />
                                    <button
                                ...
                                <>
                                    <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">{name || user?.name || 'Vendor Name'}</h2>
                                    <button
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">{user?.email || 'vendor@email.com'}</p>
                    </div>
                </div>
            </motion.div>
                    ...
                    >
                            ...
                            <div className="space-y-3">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-9 pr-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-400 transition-all"
                                    />
                                </div>
                                <button
                        ...
                        <div className="flex gap-2 mt-4">
                            ...
                            <button
                                ...
                                className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 text-white font-semibold text-sm shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/35 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
                            >
                                {isLoading ? (
            ...
            >
                {[
                    { icon: User, label: 'Full Name', value: name || user?.name, color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-900/20' },
                    { icon: Mail, label: 'Email Address', value: user?.email, color: 'text-teal-500', bg: 'bg-teal-50 dark:bg-teal-900/20' },
                    { icon: Shield, label: 'Account Role', value: 'Seller', color: 'text-cyan-500', bg: 'bg-cyan-50 dark:bg-cyan-900/20' },
                    { icon: Calendar, label: 'Member Since', value: joinDate, color: 'text-sky-500', bg: 'bg-sky-50 dark:bg-sky-900/20' },
                ].map((item, index) => {
            ...
            >
                ...
                <div className="flex flex-wrap gap-3">
                    <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800">
                        ...
                        <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400">Seller Account Active</span>
                    </div>
                    <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-teal-50 dark:bg-teal-900/20 border border-teal-100 dark:border-teal-800">
                        <Shield className="w-3.5 h-3.5 text-teal-500" />
                        <span className="text-xs font-medium text-teal-600 dark:text-teal-400">Storefront Access Enabled</span>
                    </div>
                    {savedImage && (
